...
int main(int argc, char **argv) {
    ...
    const char *algo_name = "non_blocking_isend_irecv";
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        if (rank == 0) {
            ...
            if (size > 1) {
                ...
                unsigned long long *bufs = malloc(sizeof(unsigned long long) * (size - 1));
                for (int src = 1; src < size; ++src) {
                    MPI_Irecv(&bufs[src-1], 1, MPI_UNSIGNED_LONG_LONG, src, 321, MPI_COMM_WORLD, &reqs[src-1]);
                }
                MPI_Waitall(size - 1, reqs, MPI_STATUSES_IGNORE);
                for (int i = 0; i < size - 1; ++i) sum += bufs[i];
                free(reqs); free(bufs);
            }
            global_inside = sum;
        } else {
            MPI_Request req;
            MPI_Isend(&local_inside, 1, MPI_UNSIGNED_LONG_LONG, 0, 321, MPI_COMM_WORLD, &req);
